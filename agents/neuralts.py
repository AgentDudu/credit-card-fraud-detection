...
class NeuralTSAgent:
    def __init__(self, n_actions, n_features, v=0.1, hidden_dim=32, lr=0.001, memory_size=2000, batch_size=32):
    ...
    def get_action(self, context):
        ...
        for a in range(self.n_actions):
            ...
            variance = (features_np.T @ self.A_inv[a] @ features_np).item()
            sigma = np.sqrt(max(variance, 1e-9))
            
            sampled_reward = np.random.normal(loc=expected_reward.item(), scale=self.v * sigma)
            p[a] = sampled_reward
